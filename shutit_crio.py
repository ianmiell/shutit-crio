...
WantedBy=multi-user.target" > /etc/systemd/system/crio.service'""")
		...
		pod_id = shutit.send_and_get_output('crictl runs /root/go/src/github.com/kubernetes-incubator/cri-o/test/testdata/sandbox_config.json')
		if pod_id == '' or pod_id.find(' ') != -1:
			shutit.pause_point('no pod?')
		shutit.send('crictl inspects --output table ' + pod_id)
		container_id = shutit.send_and_get_output('crictl create ' + pod_id + ' /root/go/src/github.com/kubernetes-incubator/cri-o/test/testdata/container_config.json /root/go/src/github.com/kubernetes-incubator/cri-o/test/testdata/sandbox_config.json')
		if container_id == '' or container_id.find(' ') != -1:
			shutit.pause_point('no container_id?')
